...
The image below is form the [Istio documentation](https://istio.io/docs/concepts/what-is-istio/) and shows the basic Istio architecture.

![Istio architecture](images/traffic-routing-deployment04.svg)

The following image shows a simplified view on the given information for our situation. The Pod's do have **injected as additional container**.

![injected as additional containe](images/traffic-routing-deployment05.png)

Now we want to control the  route traffic (e.g. REST API calls) into a Kubernetes application normally requires a **Kubernetes Ingress**. With Istio, the equivalent is a **Istio Gateway** which allows it to manage and monitor incoming traffic. This gateway in turn uses the Istio ingress gateway which is a pod running in Kubernetes. This is the definition of an Istio gateway. The [Istio ingress.ymal](web-api-java-jee/deployment/istio-ingress.yaml).
...
![Istio gateway gif](images/traffic-routing-deployment08.png)
