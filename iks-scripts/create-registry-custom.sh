...
readonly LOG_FILE=${root_folder}/iks-scripts/create-registry-custom.log 
touch $LOG_FILE
...
function create_registry() {
  ...
  ibmcloud cr namespace-list >> $LOG_FILE 2>&1
  ...
  if [ $? == 0 ]; then 
    ...
    _out SOMETHING WENT WRONG! Check the iks-scripts/create-registry-custom.log 
  fi
}
